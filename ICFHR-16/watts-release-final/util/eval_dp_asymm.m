...
if(strcmp(opts.dataset,'Bentham')==1 && opts.test ==1 ) %% Use only selected queries for Bentham dataset
    query = textread('/home/cvit/kartik/watts-release/data/queries.txt','%s');
    keep_bentham = find(ismember(lower(queriesWords), lower(query)));
    keep = intersect(keep,keep_bentham);
end

if(opts.invocab == 1 && strcmp(opts.dataset,'IAM')==1)
	%keyboard 
	import_nintyk
	load IAM-train-labels
        load 10k-index

	dictionary = union(lower(nintyk(folder_name)), lower(train_label));
	keep_invocab = find(ismember(lower(queriesWords), lower(dictionary)));
	keep = intersect(keep_invocab,keep);
end

if(opts.outvocab == 1 && strcmp(opts.dataset,'IAM')==1)
        import_nintyk
	load IAM-train-labels
        load 10k-index

        dictionary = union(lower(nintyk(folder_name)), lower(train_label));
        keep_outvocab = find(~ismember(lower(queriesWords), lower(dictionary)));
        keep = intersect(keep_outvocab,keep);
end
